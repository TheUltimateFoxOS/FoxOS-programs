...
int main(int argc, char* argv[]) {
	...
	file_scanner_t* file_scanner = find_file_scanner(buffer, f->size);

	if (file_scanner == NULL) {
		printf("Unable to identify file type\n");
	} else {
		char additional_information[512] = { 0 };
		file_scanner->get_information(buffer, f->size, additional_information, sizeof(additional_information));
